...
public class jHeapPanel extends JPanel {
    ...
    private int drawVertex(Graphics g, Node vertical, int x, int y, int xO, int yO) {
	int delta = 0;
	Node temp = vertical;
	do {
	    if (xO > 0 && yO > 0) {
	    ...
	    g.drawString(Double.toString(temp.getKey()), x + delta + jHeapPanel.radius, y + jHeapPanel.radius/2);
	    if (i>0)
		delta += i;
	    else
